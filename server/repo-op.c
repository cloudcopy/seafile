...
free_file_info (gpointer info)
{
    ...
    g_hash_table_destroy (file_info->dir_ids);
    g_free (file_info);
}

// judge whether dir_ids contain id,
// if contain then it indicate file is not changed else cache id
gboolean
compare_or_add_id (GHashTable *dir_ids,
                   GHashTable *cur_dir_ids,
                   const char *id)
{
    gboolean ret = FALSE;

    if (dir_ids == NULL) {
        g_hash_table_add (cur_dir_ids, g_strdup (id));
    } else if (g_hash_table_contains (dir_ids, id)) {
        ret = TRUE;
    } else {
        g_hash_table_add (cur_dir_ids, g_strdup (id));
    }
                     ...
                     GError **error)
{
    ...
    if (compare_or_add_id (dir_ids, cur_dir_ids, dir_id)) {
        // Actually there is not error occur, just indicate file doesn't changed
        g_set_error (error, SEAFILE_DOMAIN, SEAF_ERR_GENERAL, "File doesn't changed");
        goto out;
    }
               ...
               GError **error)
{
    ...
    GHashTable *cur_dir_ids = g_hash_table_new_full (g_str_hash, g_str_equal,
                                                     g_free, NULL);
...
out:
    if (dir)
        seaf_dir_free (dir);
    if (!file_info)
        g_hash_table_destroy (cur_dir_ids);
    g_free (file_name);
    g_free (dir_name);
...
collect_file_revisions (SeafCommit *commit, void *vdata, gboolean *stop)
{
    ...
    parent1_info = get_file_info (data->repo, parent_commit, path,
                                  file_info_cache, file_info->dir_ids, error);
    if (*error) {
        ret = FALSE;
        goto out;
    }
